...
crea_file_scenario <- function(
  ...
  liste_future <- data.table(LISTA_FUTURA = character(), COALIZIONE = character(), COLORE = character())
  
  # --- 2. flussi_previsti ------------------------------------------
  colonne_flussi <- c(
    "DATA", "ELEZIONE", "LISTA"
  )
  flussi_previsti <- unique(dati[, ..colonne_flussi])
