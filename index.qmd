...
carica_dati <- function(ramo) {
  ...
  # Assegno un nome ai candidati delle liste "altre"
  liste_uni$CANDIDATO[is.na(liste_uni$CANDIDATO)] <- 
      seq_len(sum(is.na(liste_uni$CANDIDATO)))
  ...
  candidati_omonimi <- 
    candidati_uni[
      candidati_uni$CANDIDATO %in% 
        candidati_uni$CANDIDATO[duplicated(candidati_uni$CANDIDATO)], 
    ]
  liste_uni[liste_uni$CANDIDATO %in% candidati_omonimi$CANDIDATO, ]
  
  list(
    candidati_uni = liste_uni, 
...
simula <- function(
  ...
  # Copio i candidati uninominali da candidati_uni a liste_uni
  ...
  # Creo candidati fittizi per le altre liste
  dati$liste_uni$CANDIDATO[is.na(dati$liste_uni$CANDIDATO)] <- 
      seq_len(sum(is.na(dati$liste_uni$CANDIDATO)))
  
  # Ricreo il dataframe candidati_uni
  dati$candidati_uni <- unique(
    dati$liste_uni[
      , 
      c(
        "CIRCOSCRIZIONE",
