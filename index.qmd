...
log_astensione <- 
  mean(qlogis(liste_sondaggi$PERCENTUALE[liste_sondaggi$ABBREV == "astenuti"]))
log_astensione_sd <- 
  sd(qlogis(liste_sondaggi$PERCENTUALE[liste_sondaggi$ABBREV == "astenuti"]))
...
simula <- function(
  ...
  iterazione <- function(
    ...
    #### Simulazione percentuali nazionali ####
    liste_naz$LOG_P <- rnorm(
      liste_naz$PERCENTUALE_LOG,
      liste_naz$PERCENTUALE_LOG,
      liste_naz$PERCENTUALE_LOG_SD
    )
    
    liste_naz$PERCENTUALE <- exp(liste_naz$LOG_P) / sum(exp(liste_naz$LOG_P))
    
    astensione <- plogis(rnorm(1, log_astensione, log_astensione_sd))
    
    #### Calcolo percentuali per provincia ####

    liste_naz$PERC_CORRETTA <-liste_naz$PERCENTUALE * (1 - astensione) 
    liste_naz <- merge(
      liste_naz,
      data.frame(
        AREA = "astensione",
        PERC_CORRETTA = astensione
      ),
      all = TRUE
    )
    ...
    if (ramo == "camera") {
      colonna_circ <- comuni_aree$CIRCOCAM_20_DEN
      colonna_pluri <- comuni_aree$CP20_DEN
      colonna_uni <- comuni_aree$CU20_DEN
    }
    
    if (ramo == "senato") {
      colonna_circ <- comuni_aree$DEN_REG20
      colonna_pluri <- comuni_aree$SP20_DEN
      colonna_uni <- comuni_aree$SU20_DEN
    }
    
    dati$aree_uni <- aggregate(
      cbind(VOTANTI, POP_2011) ~ colonna_circ + colonna_pluri + colonna_uni + AREA,
      comuni_aree,
      sum
    )
    
    names(dati$aree_uni)[1] <- "CIRCOSCRIZIONE"
    names(dati$aree_uni)[2] <- "COLLEGIOPLURINOMINALE"
    names(dati$aree_uni)[3] <- "COLLEGIOUNINOMINALE"
  ...
  lista_risultati <- parLapply(
    ...
    dati = dati,
    liste_naz = liste_naz,
    log_astensione = log_astensione,
    log_astensione_sd = log_astensione_sd,
    aree = aree,
...
Sono stati considerati i seguenti sondaggi:

```{r liste_naz}

options(knitr.kable.NA = '')

liste_sondaggi$PERCENTUALE <- 
  formattable::percent(liste_sondaggi$PERCENTUALE, 1)

liste <- reshape(
  as.data.frame(liste_sondaggi),
  idvar = "ABBREV",
  timevar = "SONDAGGIO",
  drop = "DATA",
  direction = "wide"
...
names(liste) <- str_remove(names(liste), "PERCENTUALE\\.")
liste$medie <- apply(liste[-1], 1, mean, na.rm = TRUE)
liste$medie[liste$ABBREV == "altri"] <- -1
liste$medie[liste$ABBREV == "astenuti"] <- -2

liste <- merge(
  liste, 
  liste_naz[, c("ABBREV", "COALIZIONE", "AREA")],
  all.x = TRUE
)

liste <- liste[order(liste$medie, decreasing = TRUE), ]

names(liste)[names(liste) == "ABBREV"] <- "Lista"
names(liste)[names(liste) == "COALIZIONE"] <- "Coal."
names(liste)[names(liste) == "AREA"] <- "Area"


kbl(
  liste[, -which(names(liste) == "medie")],
  row.names = FALSE
) %>%
  kable_minimal()
...
politica.
5. Sono state effettuate `r iterazioni` simulazioni. In ciascuna di esse:
   1. La percentuale nazionale di ciascuna lista è stata estratta a sorte
   da una distribuzione che ha una media e una deviazione standard
   ricavate dai sondaggi considerati.
   2. Per ciascuna area politica, i voti nazionali (calcolati in base alle 
   percentuali dei sondaggi mostrate nella tabella sopra) sono stati divisi nelle 
   diverse provincie in base alla distribuzione calcolata al punto 4.
   3. Sono state calcolate le percentuali di ogni area all'interno di ogni 
   provincia, rispetto al totale di voti assegnati a ogni provincia.
   4. A partire dalle percentuali del punto 6 sono state calcolate le percentuali
   ...
   5. In ogni collegio, le percentuali di ciascuna area sono state suddivise tra le
   liste di quell'area proporzionalmente alla percentuale nazionale (da sondaggio) 
   di ciascuna lista.
   6. La percentuale di ciascuna lista in ciascun collegio è stata fatta
   variare leggermente rispetto a quella calcolata al punto precedente,
   la variabilità di questa percentuale è stata ricavata dalle
   precedenti elezioni.
